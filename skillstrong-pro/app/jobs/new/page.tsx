...
title: '', company: '', location: '', description: '', skills: '', payMin: '', payMax: '', apprenticeship: false, featured: false,
});
...
if (payload.featured) {
await fetch('/api/featured', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ kind: 'job', refId: data.job.id, categoryHint: form.title, metroHint: form.location }) });
}
r.push('/jobs');
};
